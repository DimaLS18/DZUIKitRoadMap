...
class PizzaDetailViewController: UIViewController {
    ...
    private lazy var bucketButton: UIButton = {
     ...
     private var isHamAdded = false {
         didSet {
             setupTopping(imageView: firstToppingImageView, isAdded: isHamAdded)
         }
     }
     ...
     private var isPepperAdded = false {
         didSet {
             setupTopping(imageView: thirdToppingImageView, isAdded: isPepperAdded)
         }
     }
    ...
    private var totalPizzaPrice = 350.00 {
     ...
     @objc private func addToppingAction(_ sender: UITapGestureRecognizer) {
         switch sender.view {
         case firstToppingImageView: isHamAdded.toggle()
         case secondToppingImageView: isChilliAdded.toggle()
         case thirdToppingImageView: isPepperAdded.toggle()
         default: break
         }
     }
     ...
     @objc private func bucketButtonAction() {
         ...
         bucketVC.setupOrderData(orders: pizzaName,
                                 isHamAdded ? "СЫР" : nil,
                                 isPepperAdded ? "ХАЛАПЕНЬО" : nil,
                                 isChilliAdded ? "БЕКОН" : nil)
